;; math_sum.wat
;; Parses first two integers in input JSON and returns {"sum":<value>}.
;; Errors are returned as {"error":"invalid_input"}.

(module
  (memory (export "memory") 3)

  (data (i32.const 131072) "{\22sum\22:")
  ...
  (func (export "run")
    ...
    (if (i32.lt_u (local.get $outCap) (i32.const 29))
      (then (return (i32.const 0)))
    )

    (local.set $n (call $write_const (local.get $outPtr) (i32.const 131072) (i32.const 7)))
    (local.set $n
      (i32.add
        (local.get $n)
        (call $write_i64
