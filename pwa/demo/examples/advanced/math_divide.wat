;; math_divide.wat
;; Parses first two integers in input JSON and returns {"quotient":<value>}.
;; Errors are returned as {"error":"invalid_input"} and {"error":"divide_by_zero"}.

(module
  (memory (export "memory") 3)

  (data (i32.const 131072) "{\22quotient\22:")
  (data (i32.const 131100) "}")
  (data (i32.const 131120) "{\22error\22:\22invalid_input\22}")
  (data (i32.const 131160) "{\22error\22:\22divide_by_zero\22}")
  ...
  (func (export "run")
    ...
    (if (i64.eq (local.get $b) (i64.const 0))
      (then
        (if (i32.lt_u (local.get $outCap) (i32.const 26)) (then (return (i32.const 0))))
        (drop (call $write_const (local.get $outPtr) (i32.const 131160) (i32.const 26)))
        (return (i32.const 26))
      )
    )
    ...
    (if
      ...
      (else
        (local.set $result (i64.div_s (local.get $a) (local.get $b)))
      )
    )

    (if (i32.lt_u (local.get $outCap) (i32.const 34))
      (then (return (i32.const 0)))
    )

    (local.set $n (call $write_const (local.get $outPtr) (i32.const 131072) (i32.const 12)))
    (local.set $n
      (i32.add
        (local.get $n)
        (call $write_i64
