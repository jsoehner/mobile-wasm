;; math_subtract.wat
;; Parses first two integers in input JSON and returns {"difference":<value>}.
;; Errors are returned as {"error":"invalid_input"}.

(module
  (memory (export "memory") 3)

  (data (i32.const 131072) "{\22difference\22:")
  ...
  (func (export "run")
    ...
    (local.set $result (i64.sub (local.get $a) (local.get $b)))
    (if
      (i32.and
        (i64.lt_s (local.get $b) (i64.const 0))
        (i64.lt_s (local.get $result) (local.get $a)))
      (then (local.set $result (i64.const 9223372036854775807)))
    )
    (if
      (i32.and
        (i64.gt_s (local.get $b) (i64.const 0))
        (i64.gt_s (local.get $result) (local.get $a)))
      (then (local.set $result (i64.const -9223372036854775808)))
    )
    ...
    (local.set $n (call $write_const (local.get $outPtr) (i32.const 131072) (i32.const 14)))
    (local.set $n
      (i32.add
        (local.get $n)
        (call $write_i64
